...
#            y[gt_indices] = gt_labels[gt_indices]
    ...
    def generate_output(self, x, gt_labels):
        ...
        if self.training and np.random.rand() >= 0.5:
            ...
            pred_labels = self.inference(x, init_labels, gt_labels=None, num_iterations= iteration)
        elif self.training:
            # In training: If add_ground_truth=True, add ground truth outputs
            # to provide some positive examples to the network
            iteration = random.randint(0,30)
            init_labels = self.get_ini_labels(x, gt_labels=gt_labels)
            pred_labels = self.inference(x, init_labels, gt_labels=gt_labels, num_iterations= iteration)
        else:
            iteration = 30
            init_labels = self.get_ini_labels(x)
            pred_labels = self.inference(x, init_labels, num_iterations= iteration)

        return pred_labels

    def inference(self, x, y, gt_labels=None, num_iterations = 30):

        if self.training:
            self.model.eval()
        
        #optim_inf = SGD(y, lr=3, momentum=0.9)
        optim_inf = Adam(y, lr= 10)
...
#                # Project back to the valid range
            ...
            #visualize all the inference process while validation
            if not self.training:
                img = y.detach().cpu()
                directory = "./validation/epoch" + str(self.epoch) + "_" + str(self.filename)
                self.filename += 1
...
#%%using later in the future for efficiency
...
if __name__ == "__main__":
    ...
    DVN = DeepValueNetwork(WhorseDataset, use_cuda,
                           ...
                           batch_size=16, batch_size_eval=16, non_linearity=nn.ReLU())
